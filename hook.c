...
uint32_t find_got_entry_address(const char *module_path, const char *symbol_name) {
   ...
   //ALOGI("[+] base address of %s: %x", module_path, module_base);

   int fd = open(module_path, O_RDONLY);
   if ( fd == -1 ) {
       //ALOGE("[-] open %s error!", module_path);
       return 0;
   }

   Elf32_Ehdr *elf_header = (Elf32_Ehdr *)malloc(sizeof(Elf32_Ehdr));
   if ( read(fd, elf_header, sizeof(Elf32_Ehdr)) != sizeof(Elf32_Ehdr) ) {
       //ALOGE("[-] read %s error! in %s at line %d", module_path, __FILE__, __LINE__);
       return 0;
   }
   ...
   //ALOGI("[+] start of section headers: %x", sh_base);
   //ALOGI("[+] section header string table index: %d", ndx);
   //ALOGI("[+] section header of section header string table offset: %x", shstr_base);
   ...
   if ( read(fd, shstr_shdr, sizeof(Elf32_Shdr)) != sizeof(Elf32_Shdr) ) {
    ...
    //ALOGI("[+] section header string table offset: %x", shstr_shdr->sh_offset);
    ...
    for ( ; i < elf_header->e_shnum; i++ ) {
        ...
        if ( read(fd, shdr, sizeof(Elf32_Shdr)) != sizeof(Elf32_Shdr) ) {
            //ALOGE("[-] read %s error! i = %d, in %s at line %d", module_path, i, __FILE__, __LINE__);
            return 0;
        }
    }

    /*
    ALOGI("[+] offset of .rel.plt section: %x", relplt_shdr->sh_offset);
    ALOGI("[+] offset of .dynsym section: %x", dynsym_shdr->sh_offset);
    ALOGI("[+] offset of .dynstr section: %x", dynstr_shdr->sh_offset);
    */
    ...
    if ( read(fd, dynstr, dynstr_shdr->sh_size) != dynstr_shdr->sh_size ) {
        //ALOGE("[-] read %s error!", module_path);
        return 0;
    }
    ...
    for (i = 0; i < relplt_shdr->sh_size / sizeof(Elf32_Rel); i++ ) {
        ...
        if ( strcmp(dynstr + dynsymtab[ndx].st_name, symbol_name) == 0 ) {
            //ALOGI("[+] got entry offset of %s: %x", symbol_name, rel_ent->r_offset);
            break;
        }
        if ( read(fd, rel_ent, sizeof(Elf32_Rel)) != sizeof(Elf32_Rel) ) {
            //ALOGE("[-] read %s error!", module_path);
...
/**
 * replace GOT entry content of function that indicated by func_name
